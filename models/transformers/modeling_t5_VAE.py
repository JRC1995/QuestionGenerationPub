...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _shift_right(self, input_ids, type_ids=None):
        ...
        if type_ids is None:
            ...
            shifted_input_ids = torch.cat([type_ids, shifted_input_ids[..., 1:]], dim=-1)
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.prior_mean_var1 = nn.Linear(config.d_model, config.d_model)
        self.prior_mean_var21 = nn.Linear(config.d_model, config.d_model)
        self.prior_mean_var22 = nn.Linear(config.d_model, config.d_model)

        self.posterior_mean_var1 = nn.Linear(2 * config.d_model, config.d_model)
        self.posterior_mean_var21 = nn.Linear(config.d_model, config.d_model)
        self.posterior_mean_var22 = nn.Linear(config.d_model, config.d_model)
    ...
    def get_prior_mean_var(self, x):
        h1 = F.relu(self.prior_mean_var1(x))
        return self.prior_mean_var21(h1), self.prior_mean_var22(h1)

    def get_posterior_mean_var(self, x):
        h1 = F.relu(self.posterior_mean_var1(x))
        return self.posterior_mean_var21(h1), self.posterior_mean_var22(h1)
    ...
    def forward(
        ...
        if self.z is None:
            ...
            prior_mu, prior_logvar = self.get_prior_mean_var(prior_cls_token)
            prior_z = self.reparameterize(prior_mu, prior_logvar)
            ...
            if self.training:
                ...
                posterior_cls_token = posterior_encoder_outputs[0][:, 0, :]
                posterior_mu, posterior_logvar = self.get_posterior_mean_var(torch.cat([prior_cls_token,
                                                                                        posterior_cls_token], dim=-1))
                posterior_z = self.reparameterize(posterior_mu, posterior_logvar)
                z = posterior_z

                N, d = prior_logvar.size()

                prior_covariance = torch.eye(d).view(1, d, d).to(prior_logvar.device) \
                                   * torch.exp(prior_logvar).view(N, d, 1)
                posterior_covariance = torch.eye(d).view(1, d, d).to(posterior_logvar.device) \
                                       * torch.exp(posterior_logvar).view(N, d, 1)
                # print("prior_covariance: ", prior_covariance)

                prior_dist = torch.distributions.multivariate_normal.MultivariateNormal(loc=prior_mu,
                                                                                        covariance_matrix=prior_covariance)
                posterior_dist = torch.distributions.multivariate_normal.MultivariateNormal(loc=posterior_mu,
                                                                                            covariance_matrix=posterior_covariance)

                kd = torch.distributions.kl.kl_divergence(posterior_dist, prior_dist)

                # print("kd size: ", kd.size())
                self.kd = kd.mean()
            else:
                z = prior_z
                self.kd = None
        ...
        if labels is not None and decoder_input_ids is None and decoder_inputs_embeds is None:
            ...
            if self.extra_config is not None and "type" in self.extra_config["dataset"]:
                N, _ = input_ids.size()
                type_ids = input_ids[:, 1].view(N, 1)
                decoder_input_ids = self._shift_right(labels, type_ids=type_ids)
            else:
