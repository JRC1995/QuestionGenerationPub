...
class GenerationMixin:
    ...
    def generate(
        ...
        if self.config.is_encoder_decoder:
            ...
            encoder_outputs: ModelOutput = encoder(input_ids=input_ids, attention_mask=attention_mask, return_dict=True)
        ...
        if self.config.is_encoder_decoder:
            ...
            """
            if self.extra_config is not None and "type" in self.extra_config["dataset"]:
                input_ids = type_ids.unsqueeze(1).unsqueeze(1).repeat(1, effective_batch_mult, num_beams, 1)
                input_ids = input_ids.view(effective_batch_size, 1)
            else:
            """
            input_ids = torch.full(
    ...
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            model_inputs = self.prepare_inputs_for_generation(
                input_ids=input_ids, past=past, attention_mask=attention_mask, use_cache=use_cache, **model_kwargs
            )
