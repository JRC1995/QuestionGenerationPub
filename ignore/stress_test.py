...
from models.transformers import T5Tokenizer
...
def run(args, config, time=0):
    ...
    tokenizer = T5Tokenizer.from_pretrained(config["embedding_path"])
    #collater = eval("{}_stess_collater".format(args.model_type))
    collater = Seq2Seq_stress_collater(PAD=tokenizer.pad_token_id, max_src_len=src_max_len, max_trg_len=trg_max_len)
        ...
        for epoch in range(loaded_stuff["past_epochs"], epochs):
            ...
            if args.chunk_size != -1:
                ...
                while current_iter < train_data_len:
                    ...
                    incr = args.chunk_size if current_iter + args.chunk_size <= train_data_len else train_data_len - current_iter
                    ...
                    train_items = trainer.train(epoch, train_dataloader)
...
def run_and_collect_results(args, config):
    ...
    for key in best_metrics:
        ...
        for k, v in best_metrics[key].items():
            display_string += "{}: {} (median) {} (mean) +- {} (std); ".format(k, np.median(v), np.mean(v), np.std(v))
        display_string += "\n"
