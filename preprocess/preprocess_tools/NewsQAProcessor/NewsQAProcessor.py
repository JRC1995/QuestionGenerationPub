...
def generate_question_worthiness_hierarchical_labeler_data(paragraphs, sentences):
    ...
    for paragraph in list(set(paragraphs)):
        ...
        max_sentences = 5
        start_idx = [i for i in range(0, max(1, len(sentences) - max_sentences))]
        start_id = random.choice(start_idx)
        sentences = sentences[start_id:start_id+max_sentences]
...
def generate_one2one_sentence_QG_data(sentences, questions, paragraphs):
    ...
    for sentence, paragraph, question in zip(sentences, paragraphs, questions):
        if sentence in sentences_dict and question not in sentences_dict[sentence]:
    ...
    for sentence, paragraph, question in zip(sentences, paragraphs, questions):
        ...
        sentences_ = paragraph.split("--|--")
        as_id = sentences_.index(sentence)
        if as_id > 2:
            selected_sentences = [sentences_[as_id - 2], sentences_[as_id - 1]] + sentences_[as_id:]
        else:
            selected_sentences = sentences_
...
def generate_one2many_sentence_QG_data(sentences, questions, paragraphs):
    ...
    for sentence, paragraph, question in zip(sentences, paragraphs, questions):
        if sentence in sentences_dict and question not in sentences_dict[sentence]["question"]:
    ...
    for sentence in sentences_dict:
        ...
        paragraph = sentences_dict[sentence]["paragraph"]
        sentences_ = paragraph.split("--|--")
        as_id = sentences_.index(sentence)
        if as_id > 2:
            selected_sentences = [sentences_[as_id - 2], sentences_[as_id - 1]] + sentences_[as_id:]
        ...
        sample = {"question": " <sep> ".join(sentences_dict[sentence]["question"]),
                  ...
                  "document": paragraph.replace(sentence,
                                                " <hl> " + sentence + " </hl> ")}
        samples.append(sample)
...
def generate_one2many_QG_data(paragraphs, questions):
    ...
    for paragraph, question in zip(paragraphs, questions):
        if paragraph in paragraphs_dict and question not in paragraphs_dict[paragraph]:
...
def generate_one2one_QG_data(paragraphs, questions):
    ...
    paragraph2questions = {}
    for paragraph, question in zip(paragraphs, questions):
        if paragraph in paragraph2questions and question not in paragraph2questions[paragraph]:
    ...
    for sentence, paragraph, question, question_type in zip(sentences, paragraphs, questions, question_types):
        if sentence in sentences_dict:
            if question_type in sentences_dict[sentence] \
                    and question not in sentences_dict[sentence]["question"][question_type]:
    ...
    for sentence in sentences_dict:
        for question_type in sentences_dict[sentence]["question"]:
            ...
            sample = {"question": " <sep> ".join(sentences_dict[sentence]["question"][question_type]),
                      ...
                      "document": "<" + question_type + "> " + paragraph.replace(sentence,
                                                                                 " <hl> " + sentence + " </hl> ")}
            samples.append(sample)
